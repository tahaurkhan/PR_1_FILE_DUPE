...
public class FileScanner {
    ...
    public List<FileData> scanFullSystem(Consumer<String> progressCallback) {
        ...
        for (File root : roots) {
            if (isCancelled) {
                break; // If cancelled in Pass 2, it will still return what it found!
            }
    ...
    private boolean isAllowedFolder(File folder) {
        ...
        // Critical Linux system/virtual folders that must ALWAYS be skipped to prevent infinite loops or crash
        String absPath = folder.getAbsolutePath().toLowerCase();
        if (absPath.contains("/run/") || absPath.contains("/proc/")
                || absPath.contains("/sys/") || absPath.contains("/dev/")) {
            skippedCount++;
            return false;
        }

        // Check user setting for general system file skipping
        DataStore store = new DataStore();
        if (store.isSkipSystemFiles()) {
            if (absPath.contains("/snap/") || absPath.contains("/timeshift/")) {
                skippedCount++;
                return false;
            }
